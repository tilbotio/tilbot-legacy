define("RemoteProjectServer", ['fs', 'Project', 'Models'], function(fs, Project, Models) {
  ...
  return class RemoteProjectServer {
      ...
      try {
        ...
        $.when(Project.fromJSON(JSON.parse(data))).then(function(project) {
          ...
          io.on('connection', (socket) => {
            ...
            self.clients[socket.id] = new Models.Log();
    ...
    message_sent_event(socket_id) {
      ...
      if (this.project.blocks[this.clients[socket_id].current_block_id].type == 'Auto') {
        this.clients[socket_id].current_block_id = this.project.blocks[this.clients[socket_id].current_block_id].connectors[0].targets[0];
    ...
    plan_message(socket_id, block_id) {
      ...
      setTimeout(function() {
        self.send_message(socket_id, self.project.blocks[block_id]);
      }, this.project.blocks[block_id].delay * 1000);
    }

    receive_message(socket_id, str) {
      this.clients[socket_id].messages.push(new Models.Message({message: str, source: 'user'}));
      this.clients[socket_id].save();
      var block = this.project.blocks[this.clients[socket_id].current_block_id];
